...
<p style="border: 1px dotted orange; padding:10px;">
    1. 받아오고자 하는 **데이터가 누락**되었음
	2. 위도, 경도를 받아오는 데이터 타입을 온전히 받아오기 위해 **Float이 아닌 Double 타입으로 변경** 또는 데이터 **타입을 잘못 작성**한 변수가 있어 		수정이 필요했음
	3. SQL INSERT 문을 위한 2가지 메서드를 테스트하기 위해 내가 원하는 결과에 맞게 **정확한 문자열(SQL 쿼리문)을 반환**하도록 해야했음
</p>
